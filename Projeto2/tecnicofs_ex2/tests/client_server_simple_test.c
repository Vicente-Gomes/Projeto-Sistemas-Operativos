...
int main(int argc, char **argv) {
    ...
    //printf("acabou o mount\n");

    f = tfs_open(path, TFS_O_CREAT);
    assert(f != -1);
    //printf("acabou o open\n");

    r = tfs_write(f, str, strlen(str));
    assert(r == strlen(str));
    //printf("acabou o write\n");

    assert(tfs_close(f) != -1);
    //printf("acabou o close\n");

    f = tfs_open(path, 0);
    assert(f != -1);
    //printf("acabou o segundo open\n");

    r = tfs_read(f, buffer, sizeof(buffer) - 1);
    assert(r == strlen(str));
    //printf("acabou o read\n");

    buffer[r] = '\0';
    assert(strcmp(buffer, str) == 0);

    assert(tfs_close(f) != -1);
    //printf("acabou o segundo close\n");

    assert(tfs_unmount() == 0);
    //printf("acabou o unmount\n");
